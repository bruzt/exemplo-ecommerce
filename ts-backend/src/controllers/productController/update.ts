...
export default async function update(req: Request, res: Response) {
  ...
  try {
    ...
    if (body.discount_datetime_start)
      product.discount_datetime_start = body.discount_datetime_start;
    if (body.discount_datetime_end)
      product.discount_datetime_end = body.discount_datetime_end;
    ...
    if (body.price != null) product.price = String(body.price);
    if (body.quantity_sold != null) product.quantity_sold = body.quantity_sold;
    if (body.quantity_stock != null)
      product.quantity_stock = body.quantity_stock;
    if (body.tangible) product.tangible = body.tangible;
    if (body.weight != null) product.weight = body.weight;
    if (body.height != null) product.height = body.height;
    if (body.width != null) product.width = body.width;
    if (body.length != null) product.length = body.length;
