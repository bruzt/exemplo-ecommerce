...
interface IProductWithBuyed extends Omit<ProductModel, 'calcIsOnSale'|'calcFinalPrice'|'hasId'|'save'|'remove'|'softRemove'|'recover'|'reload'> {
...
export default async function show(req: Request, res: Response) {
    ...
    try {
        ...
        const productWithBuyed: IProductWithBuyed = { ...product, productsBuyedWith: [] };
