...
export default function ListProducts() {
  ...
  const [getUpdetingProduct, setUpdetingProduct] = useState<IProduct>(
    {} as IProduct
  );
  ...
  async function fetchProducts() {
    try {
      ...
      if (Number(getSeachBar.trim())) {
        const res: AxiosResponse<IFetchProduct> = await api.get(
          `/products/${getSeachBar.trim()}`
        );
      ...
      const totalPages =
        response.data.products.length < _itemsPerPage && _currentPage == 1
          ? 1
  ...
  return (
    <Container data-testid="list-products-container">
      {getUpdeting && (
        <UpdateProduct
          product={getUpdetingProduct}
          setUpdateModalOpen={setUpdeting}
        />
      )}
      ...
      <table>
        ...
        <tbody>
          {getProducts.map((product, index) => (
            <tr key={index} data-testid="product-row">
              <td>{product.id}</td>
              <td>
