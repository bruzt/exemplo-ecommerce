...
export default function SelectAddress() {
  ...
  const [getDisabledGoToPaymentButton, setDisabledGoToPaymentButton] =
    useState(true);

  const [getIsFetchingAddAddress, setIsFetchingAddAddress] = useState(false);
  const [getIsFetchingDeleteAddress, setIsFetchingDeleteAddress] =
    useState(false);
  ...
  }, [getStreet, getNumber, getNeighborhood, getCity, getUf, getZipCode]);
  ...
  async function fetchCities() {
    try {
      const response = await axios.get(
        `https://servicodados.ibge.gov.br/api/v1/localidades/estados/${getUf}/municipios`
      );
  ...
  function handleDisabledGoToPaymentButton(addrId: number) {
    const [address] = userContext.getUser.addresses.filter(
      (address) => address.id == addrId
    );

    if (
      addrId == null ||
      !address ||
      address.zipcode != cartContext.getZipCode
    ) {
  ...
  return (
    <>
      ...
      <Container>
            ...
            return (
              <div
                ...
                className={`
                                    addr-card 
                                    ${
                                      cartContext.getAddressId == address.id
                                        ? "selected"
                                        : ""
                                    } 
                                    ${
        ...
        {getShowAddAddr && (
          <form
            className="add-addr-form"
            onSubmit={handleAddAddress}
            data-testid="add-addr-form"
          >
            ...
            <div className="flex-row">
              ...
              <div className="flex-column">
                ...
                >
                  ...
                  {getUfs.map((uf) => (
                    <option key={uf.id} value={`${uf.sigla.toUpperCase()}`}>
            ...
            <button
              className={`addr-submit ${
                getIsFetchingAddAddress && "is-fetching"
              }`}
